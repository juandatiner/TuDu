...
/// Maneja el registro, verificación y cierre de sesiones para aliados
class SessionService extends ChangeNotifier {
  static const String _deviceIdKey = 'ally_device_id';
  static const String _userEmailKey = 'logged_ally_email';
  static const String _sessionActiveKey = 'ally_session_active';
  ...
  Future<void> initialize() async {
    ...
    // Obtener email del aliado logueado
  ...
  Future<Map<String, dynamic>> checkSession(String email) async {
    try {
      final response = await http
          .post(
            Uri.parse('${Config.baseUrl}/ally-device-session/check'),
  ...
  /// Desactiva todas las demás sesiones del aliado
  Future<bool> registerSession(String email) async {
    try {
      final response = await http
          .post(
            Uri.parse('${Config.baseUrl}/ally-device-session/register'),
            headers: {'Content-Type': 'application/json'},
            body: jsonEncode({
              'email': email,
              'device_id': _deviceId,
  ...
  Future<bool> verifySessionStatus() async {
    ...
    try {
      final response = await http
          .get(
            Uri.parse(
                '${Config.baseUrl}/ally-device-session/status?email=$_userEmail&device_id=$_deviceId'),
          )
          .timeout(const Duration(seconds: 10));
  ...
  Future<void> logout() async {
    ...
    try {
      await http
          .post(
            Uri.parse('${Config.baseUrl}/ally-device-session/logout'),
            headers: {'Content-Type': 'application/json'},
            body: jsonEncode({
              'email': _userEmail,
              'device_id': _deviceId,
  ...
  /// Obtiene todas las sesiones activas del aliado
  Future<List<Map<String, dynamic>>> getActiveSessions() async {
    ...
    try {
      final response = await http
          .get(
            Uri.parse(
                '${Config.baseUrl}/ally-device-session/list?email=$_userEmail'),
          )
          .timeout(const Duration(seconds: 10));
  ...
  Future<int> closeOtherSessions() async {
    ...
    try {
      final response = await http
          .post(
            Uri.parse('${Config.baseUrl}/ally-device-session/close-others'),
            headers: {'Content-Type': 'application/json'},
            body: jsonEncode({
              'email': _userEmail,
              'device_id': _deviceId,
